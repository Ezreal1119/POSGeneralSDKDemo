...
import com.example.posdemo.databinding.FragmentIcCardBinding
import com.example.posdemo.enums.ApduCommand
import com.example.posdemo.enums.CardType
...
class IcCardFragment : Fragment() {
    ...
    private val mICCardReaderManager
        get() = (requireActivity() as CardReaderActivity).mICCardReaderManager
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnSleCard.setOnClickListener { startActivity(Intent(requireContext(), SleCardActivity::class.java)) }

        binding.spCardType.adapter = ArrayAdapter(requireContext(), simple_spinner_item, cardTypeArray).apply {
            setDropDownViewResource(simple_spinner_dropdown_item)
        }
        binding.spApdu.adapter = ArrayAdapter(requireContext(), simple_spinner_item, apduCommandArray).apply {
            setDropDownViewResource(simple_spinner_dropdown_item)
        }
        uiRefreshOnPowerOff()
    ...
    private fun onStartSearchButtonClicked() {
        ...
        Toast.makeText(requireContext(), "Please insert ICCard", Toast.LENGTH_SHORT).show()
    ...
    private fun onPowerOnButtonClicked() {
        if (!mICCardReaderManager.isCardIn) {
            Toast.makeText(requireContext(), "Please insert the ICCard first", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        runCatching {
            ...
            Toast.makeText(requireContext(), "Powered on ICC successfully", Toast.LENGTH_SHORT).show()
    ...
    private fun onPowerOffButtonClicked() {
        if (!mICCardReaderManager.isCardIn) {
            Toast.makeText(requireContext(), "Card has been removed!", Toast.LENGTH_SHORT).show()
            uiRefreshOnPowerOff()
            return
        }
        val selectedCardType = binding.spCardType.selectedItem as CardType
        runCatching {
            ...
            uiRefreshOnPowerOff()
            Log.e(TAG, "Power off successfully")
            ...
            when (apduStr) {
                ...
                ApduCommand.READ_RECORD_SFI_1_R_1.apduStr -> {
                    ...
                    selectAidDynamic = buildString {
                        append("00A40400")
                        append(CardUtil.extractTlvLenHex(BytesUtil.bytes2HexString(rspData), "4F"))
                        append(CardUtil.extractTlvValue(BytesUtil.bytes2HexString(rspData), "4F"))
                        append("00")
                    }
                    binding.tvResult.apply {
                        append("Note: \n")
                ...
                selectAidDynamic -> {
                    ...
                    val pdol = CardUtil.extractTlvValue(BytesUtil.bytes2HexString(rspData), "9F38") ?: ""
                    gpoDynamic = buildString {
                        append("80A800000C83")
                        append(CardUtil.intTo1ByteHex(CardUtil.pdolTotalBytes(pdol)))
                        append(CardUtil.randomNumberString(CardUtil.pdolTotalBytes(pdol)))
                        append("00")
                    }
                    binding.tvResult.apply {
                        append("Note: \n")
        ...
        if (!mICCardReaderManager.isCardIn) {
            detectThread = Thread {
                ...
                while (!Thread.currentThread().isInterrupted) {
                    if (mICCardReaderManager.isCardIn) {
                        ...
                        requireActivity().runOnUiThread { listener.onDetectCardIn() }
                        return@Thread
                    }
                    runCatching {
                        Thread.sleep(50)
            ...
            if (isReadingCard) {
                Thread {
                    while(!Thread.currentThread().isInterrupted) {
                        if (!mICCardReaderManager.isCardIn) {
                            requireActivity().runOnUiThread { listener.onDetectCardOut() }
                            return@Thread
                        }
                        runCatching {
                            Thread.sleep(50)
    ...
    private val mCardDetectListener = object : CardDetectListener {
        override fun onDetectCardIn() {
            binding.btnStopSearch.isEnabled = false
            Toast.makeText(requireContext(), "Card detected", Toast.LENGTH_SHORT).show()
            val atrBuffer = ByteArray(64)
            val selectedCardType = binding.spCardType.selectedItem as CardType
            runCatching {
                val outputLen = mICCardReaderManager.powerUp(selectedCardType.slot, atrBuffer)
        ...
        override fun onTimeout() {
            Toast.makeText(requireContext(), "Timeout=20s!", Toast.LENGTH_SHORT).show()
            uiRefreshOnPowerOff()
        }
    }
}
