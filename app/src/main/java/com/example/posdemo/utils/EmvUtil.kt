...
import com.example.posdemo.enums.AppTag
import com.example.posdemo.enums.CapkTag
import com.example.posdemo.enums.TerminalTag
import com.example.posdemo.enums.TransactionTag
import com.example.posdemo.fragments.emv.DEFAULT_TERMINAL_COUNTRY_CODE
import com.example.posdemo.fragments.emv.DEFAULT_TERMINAL_CAPABILITIES
import com.example.posdemo.fragments.emv.DEFAULT_TERMINAL_TYPE
import com.example.posdemo.fragments.emv.DEFAULT_THRESHOLD_RANDOM_SWITCH
import com.example.posdemo.fragments.emv.KEY_TERMINAL_CAPABILITIES
import com.example.posdemo.fragments.emv.KEY_TERMINAL_COUNTRY_CODE
import com.example.posdemo.fragments.emv.KEY_TERMINAL_TYPE
import com.example.posdemo.fragments.emv.KEY_THRESHOLD_RANDOM_SWITCH
...
/*
Notes:
 - You can set TerminalCapabilities in both updateAID(Only applicable to specific App) / updateTerminalParameters(Global).
 - TerminalCapabilities has the higher priority in AID than TerminalParameters(Global)
 - It's best practice to set TerminalCapabilities in Terminal Parameter, since it's loaded beforehand, won't changing each transaction
 */
...
object EmvUtil {
    ...
    fun generateTransactionParameters(enterAmountAfterRR: Boolean, amount: String, cardMode: ContantPara.CheckCardMode, emvOption: ContantPara.EmvOption,fallback: String): Hashtable<String, Any> {
        return Hashtable<String, Any>().apply {
            ...
            if (enterAmountAfterRR) {
                put(TransactionTag.ENTER_AMOUNT_AFTER_READ_RECORD.tag, true) // Can delete if for sure Amount is entered beforehand
            } else {
...
//            put(TransactionTag.DISABLE_CHECK_MSR_FORMAT.tag, true) // Disable checking the validity of the format of the Magnetic Strip Card's Track Data's Format
    ...
    fun updateTerminalParameters(sharedPreferences: SharedPreferences, mEmvKernelManager: EmvNfcKernelApi) {
        val terminalParameters = buildString {
            ...
            append(sharedPreferences.getString(KEY_TERMINAL_TYPE, DEFAULT_TERMINAL_TYPE) ?: DEFAULT_TERMINAL_TYPE) // Just a label, no effect. 0x22 means support Online transaction. 0x14 means Offline POS.
            ...
            append("6000F0A001") // Need to refer to the Kernel Document; "60" means 0110: supports Purchase & Service

            // 2. Optional:
            append(TerminalTag.RANDOM_TRANSACTION_SWITCH.tag)
            append(TerminalTag.RANDOM_TRANSACTION_SWITCH.len)
            append(sharedPreferences.getString(KEY_THRESHOLD_RANDOM_SWITCH, DEFAULT_THRESHOLD_RANDOM_SWITCH) ?: DEFAULT_THRESHOLD_RANDOM_SWITCH) // "E0F8C8" means support all; "E0F0C8" means not support No_CVM
...
//            append("0978") // The currency is EURO
    ...
    fun addAidUpiIcc(mEmvKernelManager: EmvNfcKernelApi, threshold: String, floorLimit: String, tacDenial: String, tacOnline: String, tacDefault: String) {
        val iccAid = Hashtable<String, String>().apply {
            // 1. Must need Fields for ICC:
            put(AppTag.CARD_TYPE.tag, "IcCard") // Means ICC
            put(AppTag.AID.tag, "A000000333010101") // UnionPay Debit ICC
            put(AppTag.APP_VERSION.tag, "0030") // Will affect TVR during TRM
            put(AppTag.CONTACT_TAC_DENIAL.tag, tacDenial) // Will affect TAA result using TVR
            put(AppTag.CONTACT_TAC_ONLINE.tag, tacOnline) // Will affect TAA result using TVR
            put(AppTag.CONTACT_TAC_DEFAULT.tag, tacDefault) // Will affect TAA result using TVR
            ...
            // 2. Optional Fields for ICC:
            put(AppTag.THRESHOLD_VALUE.tag, threshold) // 000000000000 by default; Will set TVR:B4b5=1 if got selected; Only functioning if Terminal enabled RandomSelectionSwitch
            put(AppTag.TERMINAL_FLOOR_LIMIT.tag, floorLimit) // 00000000 by default; Will set TVR:B4b8=1 if Amount > Floor_Limit
            put(AppTag.TARGET_PERCENTAGE.tag, "50") // If Amount < Threshold, then 50% of the Transaction will be selected
            put(AppTag.MAX_TARGET_PERCENTAGE.tag, "99") // If Threshold < Amount < Floor, then Prop=[50, 99] will be selected
...
//            put(AppTag.DEFAULT_TDOL.tag, "9F0206") // DEFAULT TDOL(Terminal Data Object List) is 6 Bytes Amount if Card doesn't specify. The data used by Terminal to generate the HASH code
    ...
    fun addAidUpiPicc(mEmvKernelManager: EmvNfcKernelApi) {
    ...
    fun addAidVisaIcc(mEmvKernelManager: EmvNfcKernelApi, threshold: String, floorLimit: String, tacDenial: String, tacOnline: String, tacDefault: String) {
        val aid = Hashtable<String, String>().apply {
            // 1. Must need Fields for ICC:
            put(AppTag.CARD_TYPE.tag, "IcCard") // Means ICC
            put(AppTag.AID.tag, "A0000000031010") // Visa Credit
            put(AppTag.APP_VERSION.tag, "0002") // Will affect TVR during TRM
            put(AppTag.CONTACT_TAC_DENIAL.tag, tacDenial) // Will affect TAA result using TVR
            put(AppTag.CONTACT_TAC_ONLINE.tag, tacOnline) // Will affect TAA result using TVR
            put(AppTag.CONTACT_TAC_DEFAULT.tag, tacDefault) // Will affect TAA result using TVR
            put(AppTag.DEFAULT_DDOL.tag, "9F3704") // DEFAULT DDOL(Dynamic Data Object List, used to format DDA/CDA Challenge) is 4 Bytes Random Number if Card doesn't specify
...
//            put(AppTag.APP_SELECT_INDICATOR.tag, "00") // SELECT AID must fully match the Application's ID
    ...
    fun addAidMasterCardIcc(mEmvKernelManager: EmvNfcKernelApi, threshold: String, floorLimit: String, tacDenial: String, tacOnline: String, tacDefault: String) {
        val aid = Hashtable<String, String>().apply {
            // 1. Must need Fields for ICC:
            put(AppTag.CARD_TYPE.tag, "IcCard") // Means ICC
...
//            put(AppTag.APP_SELECT_INDICATOR.tag, "00") // SELECT AID must fully match the Application's ID
    ...
    fun hex6To3Bytes(hex: String): ByteArray {
        // e.g. "E0F8C8"  →  [0xE0, 0xF8, 0xC8]
        require(hex.length == 6) { "TermCap must be 6 hex chars" }
    ...
    fun bytesToHex6(bytes: ByteArray): String {
        // e.g. [0xE0, 0xF8, 0xC8]  →  "E0F8C8"
        return bytes.joinToString("") { "%02X".format(it) }
    }

    fun parseStripeInfo(hex: String): MagStripeTracks {
        val bytes = hexToBytes(hex)

        fun readTrack(marker: Int, start: Int): Pair<String?, Int> {
            if (start >= bytes.size || (bytes[start].toInt() and 0xFF) != marker) return null to start
            val len = bytes.getOrNull(start + 1)?.toInt()?.and(0xFF) ?: return null to start
            val from = start + 2
            val to = (from + len).coerceAtMost(bytes.size)
            val s = bytes.copyOfRange(from, to).toString(Charsets.US_ASCII)
            return s to to
        }

        var i = 0
        val (t1, i1) = readTrack(0xD1, i); i = i1
        val (t2, i2) = readTrack(0xD2, i); i = i2
        val (t3, i3) = readTrack(0xD3, i); i = i3

        return MagStripeTracks(t1, t2, t3)
    }

    fun parseTrack1(track1: String): Track1Fields? {
        if (!track1.startsWith("B")) return null

        val parts = track1.substring(1).split("^")
        ...
        val rest = parts[2]
        if (rest.length < 7) return null  // YYMM + SVC 至少7位

        val expiry = rest.substring(0, 4)
        val svc = rest.substring(4, 7)
        val discretionary = rest.substring(7)
    ...
    fun parseTrack2(track2: String): Track2Fields? {
        val idx = track2.indexOf('=')
        if (idx <= 0 || track2.length < idx + 1 + 4 + 3) return null
        val pan = track2.substring(0, idx)
        val expiry = track2.substring(idx + 1, idx + 1 + 4)      // YYMM
        val svc = track2.substring(idx + 1 + 4, idx + 1 + 4 + 3) // service code
        val discretionary = track2.substring(idx + 1 + 4 + 3)
        return Track2Fields(pan, expiry, svc, discretionary)
    }


}

data class MagStripeTracks(
    val track1: String?,
    val track2: String?,
    val track3: String?
)
