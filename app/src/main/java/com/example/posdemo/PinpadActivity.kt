...
import com.example.posdemo.enums.Tr31Params
import com.example.posdemo.fragments.emv.ACTION_GET_KEY
import com.example.posdemo.utils.DataUtil
import com.example.posdemo.utils.PinpadUtil
...
const val VALUE_PIN_KEY = "pin_key"
const val VALUE_PIN_DUKPT = "pin_dukpt"
...
class PinpadActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        private const val KBPK = "B28DD617072DDCFD61BD3741D7F30B02" //8AA9256A43B0861B33B4066BFF1108625A84A4584D356CA75260CB82ABAD03B7
    ...
    private fun onLoadWKButtonClicked() {
        ...
        runCatching {
            ...
            if (intent.action == ACTION_GET_KEY && retPINKey) {
                val intent = Intent().apply {
                    putExtra("result", VALUE_PIN_KEY)
                }
    ...
    private fun onDownloadDukptButtonClicked() {
        ...
        Thread {
            runCatching {
                ...
                if (intent.action == ACTION_GET_KEY) {
                    val intent = Intent().apply {
                        putExtra("result", VALUE_PIN_DUKPT)
                    }
                    setResult(RESULT_OK, intent)
                    finish()
                }
...
//        // MAC_Key at Slot 99 is being used for the encryption; It's using DES(ECB/CBC mode) as the encryption algorithm.
    ...
    private val mPinInputListener = object: PinInputListener {
        ...
        override fun onCancel() {
            ...
            runOnUiThread {
                Toast.makeText(this@PinpadActivity, "PinPad cancelled!", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        override fun onTimeOut() {
            ...
            runOnUiThread {
                Toast.makeText(this@PinpadActivity, "PinPad timeout!", Toast.LENGTH_SHORT).show()
            }
        }

        override fun onError(errorCode: Int) {
            runOnUiThread {
                Toast.makeText(this@PinpadActivity, "onError: errorCode=$errorCode", Toast.LENGTH_SHORT).show()
